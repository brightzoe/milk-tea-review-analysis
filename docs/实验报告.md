...
实验名称：奶茶店评论情感分析与服务改进建议系统    学号：2580201251102    姓名：刘子夜

## 一、实验内容

本次实验完成了一个面向奶茶店消费评论的文本分析系统。选择奶茶店评论作为分析对象，是因为这类评论通常较短，但包含口味、价格、出餐速度、服务、包装、配送、小料等明确的用户反馈，适合用程序自动汇总情感倾向并识别主要问题。

系统以 `data/reviews.csv` 为输入，对评论进行读取、清洗、情感分析、问题归因和品牌对比统计，并将结果导出为 CSV、Markdown 报告、PNG 图表和持久化的 sklearn 模型文件。

系统主要实现以下功能：

1. 读取 CSV 评论数据，校验字段、评分和情感标签是否有效。
2. 对评论文本进行标点清理、停用词过滤和关键词提取。
3. 使用情感词典、程度副词和否定词实现规则情感分析。
4. 使用 `TF-IDF + LogisticRegression` 训练监督学习情感分类器。
5. 将评分映射为情感标签，并与人工标签、规则模型结果、机器学习结果进行一致率对比。
6. 对非正面评论进行问题归因，识别口味、价格、出餐、服务、包装、配送、产品等问题。
7. 统计整体情感分布、各品牌情感情况、高频关键词和问题排行。
8. 生成分析报告、逐条分析结果表、模型评估文件和三张统计图。

代码按功能拆分为多个类：`ReviewReader` 负责数据读取，`TextCleaner` 负责文本处理，`SentimentAnalyzer` 负责规则情感分析，`ProblemAnalyzer` 负责问题归因，`StatisticsManager` 负责统计，`ReportExporter` 和 `ChartGenerator` 负责结果输出，`SystemController` 串联完整流程。这样拆分后，每个模块职责清晰，也便于编写单元测试。
...
项目依赖写在 `requirements.txt` 中，运行前可通过以下命令安装：
...
实验数据保存在 `data/reviews.csv` 中，共 500 条评论。数据字段包括评论编号、日期、品牌、产品、渠道、评论内容、评分和人工情感标签。数据来源由两部分组成：一部分来自 ASAP 公开中文餐馆评论 sample 中经过饮品场景过滤后保留的奶茶/饮品相关评论片段（约 32 条）；另一部分是人工整理的奶茶消费场景样例（约 468 条），用于覆盖甜度、冰量、小料、外卖包装等更贴近本项目的问题。ASAP 原始数据覆盖烧烤、火锅、海鲜、日料等多种餐饮场景，因此通过白名单/黑名单过滤后仅保留与奶茶、果茶、咖啡、饮料相关的句子，避免无关餐厅评论进入分析。
...
读取数据时，我没有使用字典在各模块之间传递，而是定义了 `Review` 数据类保存单条评论及后续分析结果。这样无论是补充 tokens、情感得分，还是记录问题类别，都能放到同一个对象中，代码也更容易检查。
...
数据读取模块会检查 CSV 是否包含必要字段，评分是否在 1 到 5 之间，标签是否属于“正面、中性、负面”，评论正文是否为空，避免脏数据进入后续分析流程。

### 2. 文本清洗与关键词提取

文本清洗主要由 `TextCleaner` 完成，处理步骤包括去除常见标点、合并空格、匹配词典短语、过滤停用词等。

调试时发现一个细节：品牌“爷爷喜欢茶”中包含了“喜欢”，如果不处理，程序可能会把本来中性的评论误判为正面。解决办法是在处理单条评论时，把当前评论的品牌名和产品名作为忽略项，只对真正的评论正文表达进行分析。这个问题容易在调试时被忽略，因此我专门加了回归测试，确认品牌名里的情感词不会进入评分 tokens。
...
规则方法的优点是过程直观，能够说明一条评论为什么被判成正面或负面；缺点是词典覆盖不够时会漏判，因此后面又增加了机器学习分类器作对比。
...
对应代码在 `src/statistics_manager.py` 中：
...
评分映射与人工标签一致率为 100.00%，是因为当前样例数据的 `label` 基本按评分区间构建，可作为一个参考基线，但不能代替文本情感分析。规则模型的一致率较低，主要因为规则模型更保守，容易把没有明显情感词的评论判为中性；机器学习模型在全量数据上的一致率较高，但它依赖已有标注数据，实际泛化能力仍需要更多外部数据验证。

需要说明的是，`ml_evaluation.txt` 中的 accuracy（约 95%）是 8:2 训练/测试划分后在测试集上评估的结果；而报告中“机器学习模型与人工标签一致率”（98.40%）是把模型在 80% 训练数据上训练后，对全部 500 条数据重新预测得到的整体一致率，两者评估范围不同，因此数值存在差异。
...
<p align="center">
  <img src="../output/charts/sentiment_distribution.png" width="80%">
</p>

<p align="center">
  <img src="../output/charts/brand_sentiment_compare.png" width="80%">
</p>

<p align="center">
  <img src="../output/charts/problem_ranking.png" width="80%">
</p>
...
<p align="center">
  <img src="./images/test_output.png" width="90%">
</p>
