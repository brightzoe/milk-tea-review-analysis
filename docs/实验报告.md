...
5. 对非正面评论进行问题归因，识别口味、价格、出餐、服务、包装、配送、产品等问题。
6. 统计整体情感分布、各品牌情感情况、高频关键词和问题排行。
7. 生成分析报告、逐条分析结果表、模型评估文件和三张统计图。
...
| 文件 | 说明 |
|---|---|
| `output/analyzed_reviews.csv` | 每条评论的情感结果、问题归因和模型预测结果 |
...
3. 图表中文字体可能显示异常。
   在不同 Windows 环境中，matplotlib 的中文字体配置不一定相同。程序中设置了多个字体候选，即使图表字体存在差异，也不会影响 CSV 和 Markdown 报告生成。
